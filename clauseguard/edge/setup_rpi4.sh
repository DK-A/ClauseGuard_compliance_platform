...
if [ ! -d "llama.cpp" ]; then
    ...
    mkdir build
    cd build
    # Compile with Raspberry Pi 4 ARMv8-a Cortex-A72 compiler optimization flags
    cmake -DCMAKE_BUILD_TYPE=Release \
          -DCMAKE_C_FLAGS="-march=armv8-a+crypto -mtune=cortex-a72" \
          -DCMAKE_CXX_FLAGS="-march=armv8-a+crypto -mtune=cortex-a72" ..
    make -j$(nproc)
    cd ../..
fi
...
# Write profile override to cached file
echo "RPI4" > .clauseguard_profile
